...
Texture::Texture(ID3D11Device * device, const std::string & filePath, aiTextureType type)
{
	...
	{
		HRESULT hr = DirectX::CreateDDSTextureFromFile(device, StringConverter::StringToWide(filePath).c_str(), texture.ReleaseAndGetAddressOf(), this->textureView.ReleaseAndGetAddressOf());
		if (FAILED(hr))
		{
	...
	{
		HRESULT hr = DirectX::CreateWICTextureFromFile(device, StringConverter::StringToWide(filePath).c_str(), texture.ReleaseAndGetAddressOf(), this->textureView.ReleaseAndGetAddressOf());
		if (FAILED(hr))
		{
			this->Initialize1x1ColorTexture(device, Colors::UnloadedTextureColor, type);
		}
...
{
	this->type = type;
	HRESULT hr = DirectX::CreateWICTextureFromMemory(device, pData, size, this->texture.ReleaseAndGetAddressOf(), this->textureView.ReleaseAndGetAddressOf());
